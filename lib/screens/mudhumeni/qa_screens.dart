...
class _PublicQaScreenState extends State<PublicQaScreen>
  ...
  Map<int, bool> _upvoteStates = {}; // questionId -> hasUpvoted
  bool _loading = true;
  String _sort = 'newest';
  ...
  Future<void> _load() async {
    ...
    try {
      final questions = await MudhumeniDatabaseService.getPublicQuestions(_ward);
      
      // Check upvote status for each question
      final upvoteStates = <int, bool>{};
      for (final q in questions) {
        if (q.id != null) {
          final hasUpvoted = await DatabaseService().hasUserUpvoted(_userId, q.id!);
          upvoteStates[q.id!] = hasUpvoted;
        }
      }

      setState(() {
        _questions = questions;
        _upvoteStates = upvoteStates;
        _loading = false;
      });
    } catch (e) {
      setState(() => _loading = false);
  ...
  Future<void> _handleToggleUpvote(int questionId) async {
    final currentState = _upvoteStates[questionId] ?? false;
    final currentQuestion = _questions.firstWhere((q) => q.id == questionId);
    final currentUpvotes = currentQuestion.upvotes;

    // Optimistic UI update
    setState(() {
      _upvoteStates[questionId] = !currentState;
      final index = _questions.indexWhere((q) => q.id == questionId);
      if (index != -1) {
        _questions[index] = QaQuestion(
          id: currentQuestion.id,
          authorId: currentQuestion.authorId,
          authorName: currentQuestion.authorName,
          ward: currentQuestion.ward,
          targetMudhumeniId: currentQuestion.targetMudhumeniId,
          isPublic: currentQuestion.isPublic,
          question: currentQuestion.question,
          answer: currentQuestion.answer,
          answeredBy: currentQuestion.answeredBy,
          answeredByMudhumeni: currentQuestion.answeredByMudhumeni,
          upvotes: !currentState ? currentUpvotes + 1 : (currentUpvotes > 0 ? currentUpvotes - 1 : 0),
          madePublic: currentQuestion.madePublic,
    ...
    try {
      if (currentState) {
        // Remove upvote
        await DatabaseService().removeUpvote(_userId, questionId);
        await MudhumeniDatabaseService.decrementQuestionUpvotes(questionId);
      } else {
        // Add upvote
        await DatabaseService().addUpvote(_userId, questionId);
        await MudhumeniDatabaseService.incrementQuestionUpvotes(questionId);
      }
    } catch (e) {
      // Revert on error
      setState(() {
        ...
        children: [
          _QuestionsTab(
            ...
            onAnswer: (id, ans, by, byMudhumeni) async {
              await MudhumeniDatabaseService.answerQuestion(id, ans, by, byMudhumeni);
              _load();
            },
          ),
          _AskTab(
...
class _PrivateQaScreenState extends State<PrivateQaScreen>
  ...
  Future<void> _load() async {
    ...
    if (_isMudhumeniOrAdmin) {
      final questions = await MudhumeniDatabaseService.getPrivateQuestionsForMudhumeni(_userId);
      setState(() {
        _questions = questions;
        _loading = false;
      });
    } else {
      final wardMudhumeni = await MudhumeniDatabaseService.getVerifiedMudhumeniByWard(_ward);
      ...
      final questions = _selectedMudhumeni != null
          ? await MudhumeniDatabaseService.getPrivateQuestions(_userId, _selectedMudhumeni!.userId)
          : <QaQuestion>[];
      setState(() {
        ...
        body: Center(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.person_search_outlined, size: 64, color: AppColors.textHint),
                const SizedBox(height: 16),
                Text('No Mudhumeni in Your Ward',
                    style: AppTextStyles.heading3, textAlign: TextAlign.center),
                const SizedBox(height: 10),
                Text(
                  'There are no verified Mudhumeni extension officers in $ward yet. Check back later or use Public Q&A.',
                  ...
                  child: DropdownButtonFormField<MudhumeniProfile>(
                    ...
                    items: _wardMudhumeni
                        .map((m) => DropdownMenuItem(
                              value: m,
                              child: Text(m.fullName, style: const TextStyle(color: Colors.white)),
                            ))
                        .toList(),
                    onChanged: (m) {
                      setState(() {
        ...
        children: [
          _PrivateThreadTab(
            ...
            onAnswer: (id, ans) async {
              ...
              await MudhumeniDatabaseService.answerQuestion(id, ans, name, true);
              _load();
            },
          ),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(
            'Register your ward to connect with Mudhumeni officers and farmers in your area.',
            textAlign: TextAlign.center,
            style: AppTextStyles.body.copyWith(color: AppColors.textSecondary),
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_saving ? 'Saving...' : 'Save Ward', style: AppTextStyles.button),
            ),
          ),
        ],
      ),
...
class _QuestionsTab extends StatelessWidget {
  ...
  final Map<int, bool> upvoteStates;
  final bool loading;
  final String sort;
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: sort == s.$1 ? AppColors.primary : Colors.white,
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(
                          color: sort == s.$1 ? AppColors.primary : AppColors.divider,
                        ),
                      ),
                      child: Text(s.$2,
                          style: TextStyle(
                              fontSize: 11,
                              color: sort == s.$1 ? Colors.white : AppColors.textSecondary,
                              fontWeight: sort == s.$1 ? FontWeight.w600 : FontWeight.w400)),
                    ),
                  ),
                ),
            ],
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) => _QuestionCard(
                      q: questions[i],
                      hasUpvoted: upvoteStates[questions[i].id] ?? false,
                      onToggleUpvote: () => onToggleUpvote(questions[i].id!),
                      onAnswer: (ans, by, byMud) => onAnswer(questions[i].id!, ans, by, byMud),
                    ),
                  ),
                ),
        ),
...
// ══════════════════════════════════════════════════════════════════════════════
// QUESTION CARD (FIXED UPVOTE TOGGLE)
// ══════════════════════════════════════════════════════════════════════════════
          ...
          ElevatedButton(
            onPressed: () {
              if (ctrl.text.trim().isNotEmpty) {
                onAnswer(ctrl.text.trim(), auth.user?.fullName ?? 'User', isMudhumeniOrAdmin);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (isAnswered ? AppColors.success : AppColors.warning).withOpacity(0.12),
            ...
            Text('${q.authorName} · $dateStr', style: AppTextStyles.caption),
            if (isAnswered) ...[
              const SizedBox(height: 10),
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Icon(
                          q.answeredByMudhumeni ? Icons.verified : Icons.person_outline,
                          color: q.answeredByMudhumeni ? AppColors.success : AppColors.textSecondary,
                          size: 14,
                        ),
                        const SizedBox(width: 4),
                        Text(
                          q.answeredByMudhumeni ? '${q.answeredBy} · Mudhumeni' : q.answeredBy,
              ...
              children: [
                GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                    decoration: BoxDecoration(
                      color: hasUpvoted ? AppColors.info.withOpacity(0.12) : Colors.transparent,
                      borderRadius: BorderRadius.circular(16),
                      border: Border.all(
                        color: hasUpvoted ? AppColors.info : AppColors.divider,
                      ),
                    ),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(
                          ...
                          size: 14,
                          color: hasUpvoted ? AppColors.info : AppColors.textHint,
                        ),
                        ...
                        Text(
                          ...
                          style: AppTextStyles.caption.copyWith(
                            color: hasUpvoted ? AppColors.info : AppColors.textHint,
                            fontWeight: hasUpvoted ? FontWeight.w600 : FontWeight.w400,
                          ),
                        ),
                      ],
                    ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(q.question, style: AppTextStyles.body.copyWith(fontWeight: FontWeight.w600)),
                  if (isAnswered) ...[
                    ...
                    if (!q.madePublic && isMudhumeniOrAdmin) ...[
                      ...
                      TextButton.icon(
                        ...
                        label: const Text('Make Public', style: TextStyle(fontSize: 12)),
                      ),
                    ],
                  ] else ...[
                    const SizedBox(height: 8),
                    if (isMudhumeniOrAdmin)
                      SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          onPressed: () => _showReplyDialog(context, q.id!),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(
            'Farmers in your ward send you questions privately. Reply to them in the Thread tab. You can make answered questions public so other farmers can benefit.',
            textAlign: TextAlign.center,
            style: AppTextStyles.body.copyWith(color: AppColors.textSecondary),
          ),
        ],
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(_saving ? 'Submitting...' : 'Submit Question', style: AppTextStyles.button),
            ),
          ),
        ],
      ),
